...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-[min(calc(100vw-32px),520px)] -translate-x-1/2 -translate-y-1/2 gap-5 rounded-lg border border-slate-200 bg-white p-6 text-slate-950 shadow-xl outline-none dark:border-[#243447] dark:bg-[#0f1e2e] dark:text-[#f4f8ff]",
        className
      )}
      {...props}
    >
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description ref={ref} className={cn("text-sm leading-6 text-slate-500 dark:text-[#9ba8b7]", className)} {...props} />
));
